...
class Mover():
    ...
    def __init__ (self, input1_a_pin, input2_a_pin, pwm_a_pin, encoder_a_pin, num_a_holes, input1_b_pin, input2_b_pin, pwm_b_pin, encoder_b_pin, num_b_holes, speed_adjust_delta):
        ...
        self.left_wheel_abs_sent_speed = None               # velocidade enviada a roda, em % do pwm
        self.right_wheel_abs_sent_speed = None
    ...
    def adjustSpeed(self):
        if self.sign(self.left_wheel_required_speed) == self.sign(self.right_wheel_required_speed):
            if self.left_wheel_required_speed > 0:
                self.wheel_initial_forward_speed = (self.left_wheel_abs_sent_speed, self.right_wheel_abs_sent_speed)
            else:
                self.wheel_initial_forward_speed = (self.right_wheel_abs_sent_speed, self.left_wheel_abs_sent_speed)

        if self.left_wheel.encoder.angular_velocity > abs(self.left_wheel_required_speed):
            self.left_wheel_abs_sent_speed -= self.speed_adjust_delta
        ...
        if self.right_wheel.encoder.angular_velocity > abs(self.right_wheel_required_speed):
            self.right_wheel_abs_sent_speed -= self.speed_adjust_delta
        elif self.right_wheel.encoder.angular_velocity < abs(self.right_wheel_required_speed):
            self.right_wheel_abs_sent_speed += self.speed_adjust_delta

        self.left_wheel_abs_sent_speed = clamp(self.left_wheel_abs_sent_speed, 0, 99)
        self.right_wheel_abs_sent_speed = clamp(self.right_wheel_abs_sent_speed, 0, 99)
        self.left_wheel.spin(self.sign(self.left_wheel_required_speed)*self.left_wheel_abs_sent_speed)
        self.right_wheel.spin(self.sign(self.right_wheel_required_speed)*self.right_wheel_abs_sent_speed)
        ...
        print("Escrevendo roda direita:", self.sign(self.right_wheel_required_speed)*self.right_wheel_abs_sent_speed)
        ...
        print("Escrevendo roda esquerda:", self.sign(self.left_wheel_required_speed)*self.left_wheel_abs_sent_speed)

        self.setTimer()

    def moveForward(self, speed):
        if speed > 0:
            self.left_wheel_abs_sent_speed = self.wheel_initial_forward_speed[0]
            self.right_wheel_abs_sent_speed = self.wheel_initial_forward_speed[1]
        else:
            self.left_wheel_abs_sent_speed = self.wheel_initial_forward_speed[1]
            self.right_wheel_abs_sent_speed = self.wheel_initial_forward_speed[0]

        self.left_wheel_required_speed = speed
        self.right_wheel_required_speed = speed
        self.left_wheel.spin(self.left_wheel_abs_sent_speed)
        self.right_wheel.spin(self.right_wheel_abs_sent_speed)
    ...
    def turn(self, speed): # usando sentido positivo: antihorario
        self.left_wheel_required_speed = -speed
        self.right_wheel_required_speed = speed
        self.left_wheel_abs_sent_speed = self.wheel_initial_turn_speed
        self.right_wheel_abs_sent_speed = self.wheel_initial_turn_speed
    ...
    def stop(self):
        ...
        self.left_wheel_abs_sent_speed = 0
        self.right_wheel_abs_sent_speed = 0
